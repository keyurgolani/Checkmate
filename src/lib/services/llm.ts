...
import { createWebResearchService, type WebSearchResult } from './web-research';
...
export class LLMService {
  ...
  private async getWebResearchContext(settings: LLMSettings, query: string): Promise<string> {
    ...
    if (!webResearch.provider || !webResearch.apiKey) {
      return '';
    }
  ...
  async generateTemplate(settings: LLMSettings, query: string): Promise<LLMResult<GeneratedTemplate>> {
    try {
      ...
      const systemPrompt = `You are a checklist template generator. Given a user query, generate a comprehensive checklist template in JSON format.
...
IMPORTANT: Return ONLY the JSON object, no markdown code blocks, no explanation, just pure JSON.`;
      ...
      // Build the user prompt with optional web research context
      let userPrompt = `Create a checklist template for: ${query}`;
      if (webResearchContext) {
        userPrompt = `${webResearchContext}\n\n${userPrompt}`;
      }
      ...
      try {
        ...
        return createErrorResult({
          ...
          details: { rawResponse: text.substring(0, 500) },
        });
      }
      ...
      if (!parsed.title || !parsed.items || !Array.isArray(parsed.items)) {
        return createErrorResult({
          code: LLMErrorCodes.INVALID_RESPONSE,
          message: 'Invalid template structure in LLM response',
        });
      }
      ...
      if (parsed.questions && !Array.isArray(parsed.questions)) {
        return createErrorResult({
          code: LLMErrorCodes.INVALID_RESPONSE,
          message: 'Invalid questions structure in LLM response',
        });
      }
  ...
  async improveTemplate(
    ...
    try {
      ...
      const systemPrompt = `You are a checklist improvement expert. Analyze the given template and suggest improvements.
...
IMPORTANT: Return ONLY the JSON object, no markdown code blocks, no explanation, just pure JSON.`;
      ...
      // Build the user prompt with optional web research context
      let userPrompt = `Improve this checklist template:
...
Suggest improvements and additional steps that would make this checklist more comprehensive.`;
      ...
      let parsed: TemplateImprovements;
      try {
        const jsonMatch = text.match(/\{[\s\S]*\}/);
        if (!jsonMatch) {
          throw new Error('No JSON found in response');
      ...
      if (!parsed.suggestedItems || !Array.isArray(parsed.suggestedItems)) {
        return createErrorResult({
          code: LLMErrorCodes.INVALID_RESPONSE,
          message: 'Invalid improvement structure in LLM response',
        });
      }
  ...
  async improveStep(
    ...
    try {
      ...
      // Build context about surrounding steps
      ...
      const systemPrompt = `You are a checklist step improvement expert. Analyze the given step in context of the entire checklist and suggest improvements.
...
IMPORTANT: Return ONLY the JSON object, no markdown code blocks, no explanation, just pure JSON.`;
      ...
      // Build the user prompt with optional web research context
      let userPrompt = `Improve this checklist step:
...
Suggest improvements to make this step clearer, more actionable, and more helpful. Consider how it fits with the steps before and after it.`;
      ...
      if (!parsed.content || !parsed.description) {
        return createErrorResult({
          code: LLMErrorCodes.INVALID_RESPONSE,
          message: 'Invalid step improvement structure in LLM response',
        });
      }
  ...
  async enhanceTemplate(
    ...
    try {
      ...
      const systemPrompt = `You are a checklist enhancement expert. Your task is to IMPROVE ALL existing steps in a template by adding or improving their descriptions and resources.
...
IMPORTANT: Return ONLY the JSON object, no markdown code blocks, no explanation, just pure JSON.`;
      ...
      // Build the user prompt with optional web research context
      let userPrompt = `Enhance this checklist template by improving ALL existing steps with better descriptions and resources:
...
Provide enhanced versions of ALL ${template.items.length} steps with detailed descriptions and helpful resources.`;
      ...
      let parsed: TemplateEnhancement;
      try {
        const jsonMatch = text.match(/\{[\s\S]*\}/);
        if (!jsonMatch) {
          throw new Error('No JSON found in response');
      ...
      if (!parsed.enhancedItems || !Array.isArray(parsed.enhancedItems)) {
        return createErrorResult({
          code: LLMErrorCodes.INVALID_RESPONSE,
          message: 'Invalid enhancement structure in LLM response',
        });
      }

      // Validate that all items have IDs
      const missingIds = parsed.enhancedItems.filter(item => !item.id);
      if (missingIds.length > 0) {
        return createErrorResult({
          code: LLMErrorCodes.INVALID_RESPONSE,
          message: 'Some enhanced items are missing IDs',
        });
      }
